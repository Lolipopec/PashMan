...
- **Табличный редактор CSV** — переключатель «Текст / Таблица» (`showCsvView`). Таблица строится по
  разделителю: `parseCSVText()`/`serializeCSV()` (с поддержкой кавычек), `csvGrid` (csvGrid[0]=заголовки),
  `renderCsvTable()` рисует `<input>`-ячейки, заголовки — flex-обёртка `.hdrwrap` ВНУТРИ `<th>` (нельзя
  ставить `display:flex` на сам `<th>` — ломает раскладку строки). Любая правка → `gridToCsvText()` в `#csv`,
  поэтому прогон (читает `#csv.value`) не меняется. Смена разделителя/загрузка файла перепарсивают таблицу.
